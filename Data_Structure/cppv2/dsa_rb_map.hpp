...
namespace hzcc {
    ...
    private:
             ...
             * 更新颜色信息，x->_color应为cur->_color 根结点的颜色信息不改变，保持红黑树正确性
             * cur->_color为红色 因为(cur, x)形成了一个3结点，而3结点中小的为红色结点
             * 在x->_color = cur->_color过程中，有可能形成x->_color = cur->_color = RED
            ...
            node* min(node *cur)
            {
                    return cur->_l == nullptr ? cur : min(cur->_l);
            }
